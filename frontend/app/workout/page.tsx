...
import { Dumbbell, RefreshCw, CheckCircle2 } from 'lucide-react';
...
export default function WorkoutPlanner() {
  ...
  const fetchPlan = async () => {
    try {
      const res = await apiFetch('/api/workout/generate');
      if (res.status === 401) {
        router.push('/login');
        return;
      }
      ...
      <div className="flex-1 overflow-y-auto pl-14 pr-6 py-8 md:px-10 z-10 relative">
        ...
        {loading ? (
          ...
          <div className="glass-panel rounded-3xl p-12 text-center flex flex-col items-center justify-center border border-white/5 min-h-[400px]">
            <HeartbeatLoader text="Customizing Your AI Workout Split..." />
          </div>
